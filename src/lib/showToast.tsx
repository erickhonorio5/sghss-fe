...
const Toast = ({
    ...
    return (
        <motion.div
            ...
            className={`
        flex items-start gap-2 rounded-md p-3 text-sm shadow-md border max-w-xs
        ${
                type === 'success'
...
export const showToast = {
    success: (title: string, description?: string) => {
        return toast.custom(
            (t) => <Toast id={t.id} type="success" title={title} description={description} />,
            { duration: 3000, position: 'top-right' }
        );
    },
    error: (title: string, description?: string) => {
        return toast.custom(
            (t) => <Toast id={t.id} type="error" title={title} description={description} />,
            { duration: 3000, position: 'top-right' }
        );
    },
